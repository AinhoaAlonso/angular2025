...
describe('LibreriaService', () => {
  ...
  it('debe ser usuario válido', (done) => {
    service.autentificar("test1","test1").subscribe({
      ...
      error: (e) => {
        fail('Error en llamada autenticar: ' + e.message);
        done();
      }
    });
  });
  ...
  it('no es cliente válido', (done) => {
    service.autentificar("test44","test44").subscribe({
      ...
      error: (e) => {
        fail('Error en llamada autenticar cliente no valido: ' + e.message);
        done();
      }
    });
  });
...
describe('LibreriaService', () => {
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(LibreriaService);
    //es un suplantador, lo utilizar para evitar llamar al objeto real, en este caso para simular llamadas al backend que no existen
  ...
  it('debería comprar el libro si hay suficiente stock', () => {
    const isbn = 123;
    const unidades = 2;
    //las variables con mock para crear el supuesto stock y que devuelva el supuesto libro
